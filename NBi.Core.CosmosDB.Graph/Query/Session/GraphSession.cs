...
{
    class GraphSession : NBi.Core.Query.Session.ISession
    {
        ...
        public Type UnderlyingSessionType => typeof(GremlinSession);
        public object CreateNew() => CreateCosmosDbSession();

        public GremlinSession CreateCosmosDbSession()
        {
            return new GremlinSession(endpoint, AuthKey, DatabaseId, GraphId);
        }

        internal GraphSession(string subdomain, string api, string authKey, string databaseId, string graphId)
            : this("https", subdomain, api, 443, authKey, databaseId, graphId)
        { }

        internal GraphSession(string protocol, string subdomain, string api, int port, string authKey, string databaseId, string graphId)
            : this(protocol, subdomain, api, "azure.com", port, authKey, databaseId, graphId)
        { }

        internal GraphSession(string protocol, string subdomain, string api, string domain, int port, string authKey, string databaseId, string graphId)
            : this(new UriBuilder(protocol, $"{subdomain}.{api}.{domain}", port).Uri, authKey, databaseId, graphId)
        { }

        internal GraphSession(Uri endpoint, string authkey, string databaseId, string graphId)
        {
