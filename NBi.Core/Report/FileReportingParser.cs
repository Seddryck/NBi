...
class FileReportingParser : IReportingParser
{
    ...
    private Exception BuildDataSetNotFoundException(ReportDataSetRequest request, XmlDocument docXml, string xpath, XmlNamespaceManager nsmgr)
    {
        ...
        if (dataSetFound.Count > 1)
            throw new ArgumentException(string.Format("The requested dataset ('{2}') wasn't found for the report on path '{0}' with name '{1}'. The datasets for this report are {3}", request.Path, request.ReportName, request.DataSetName, String.Join(", ", (string[])[.. dataSetFound])));
        else
