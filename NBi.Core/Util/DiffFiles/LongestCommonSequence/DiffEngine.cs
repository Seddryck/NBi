...
namespace NBi.Core.Util.DiffFiles.LongestCommonSequence
{
    ...
    {
        private List<TextLine> _source;
        private List<TextLine> _dest;
        ...
        private int GetSourceMatchLength(int destIndex, int sourceIndex, int maxLength)
        {
            ...
            {
                if (_dest[destIndex + matchCount].CompareTo(_source[sourceIndex + matchCount]) != 0)
                {
                    break;
                }
            }
        ...
        public double ProcessDiff(List<TextLine> source, List<TextLine> destination, DiffEngineLevel level)
        {
            _level = level;
            return ProcessDiff(source, destination);
        }

        public double ProcessDiff(List<TextLine> source, List<TextLine> destination)
        {
        ...
        public IList<DiffResultSpan> DiffReport()
        {
