...
namespace NBi.Core.Util.DiffFiles.LongestCommonSequence
{
    ...
    internal class DiffStateList : List<DiffState>
    {

        public DiffStateList(int destCount) : base(destCount+1) { }

        public DiffState GetByIndex(int index)
        {
            DiffState retval = null;
            if (this.Count>index)
                retval = this[index];
            if (retval == null)
            {
	...
	public class DiffResultSpan : IComparable
	{
        ...
        public override bool Equals(object obj)
        {
            ...
            return Equals((DiffResultSpan)obj);
        }

        public bool Equals(DiffResultSpan p)
        {
            ...
            return
                (_status == p._status &&
                _destIndex == p._destIndex &&
                _sourceIndex == p._sourceIndex &&
                _length == p._length);
        }
