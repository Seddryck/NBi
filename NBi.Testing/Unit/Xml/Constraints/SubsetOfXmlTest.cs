...
namespace NBi.Testing.Unit.Xml.Constraints
{
    ...
    public class SubsetOfXmlTest
    {
        ...
        public void Deserialize_SampleFile_SubsetOfReadItems()
        {
            ...
            Assert.That(((SubsetOfXml)ts.Tests[testNr].Constraints[0]).Items[0], Is.EqualTo("First Item"));
            Assert.That(((SubsetOfXml)ts.Tests[testNr].Constraints[0]).Items[1], Is.EqualTo("Second Item"));
        }
    }
}
